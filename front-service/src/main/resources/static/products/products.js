angular.module('storage').controller('productsController', function ($scope, $http, $location, $localStorage) {
    ...
    $scope.getProducts = function (pageIndex = 1) {
        ...
        $http({
            url: contextPath + '/webapp-core/api/v1/products',
            method: 'GET',
            params: {
                p: pageIndex,
    ...
    $scope.deleteProduct = function (productId) {
        $http({
            url: contextPath + '/webapp-core/api/v1/products/delete/' + productId,
            method: 'DELETE'
    ...
    $scope.addProductInCart = function (productId) {
        $http.get(contextPath + '/webapp-cart/api/v1/cart/' + $localStorage.webappGuestCartId + '/add/' + productId)
            .then(function successCallback(response) {
            })
    }
