(function () {
    ...
    function run($rootScope, $http, $localStorage) {
        const contextPath = 'http://localhost:8888/webapp-core';
        const cartServicePath = 'http://localhost:8888/webapp-cart';
        ...
        if (!$localStorage.webappGuestCartId){
            $http.get(cartServicePath + '/api/v1/cart/generate')
...
angular.module('storage').controller('indexController', function ($rootScope, $http, $scope, $location, $localStorage) {
    const contextPath = 'http://localhost:8888/webapp-core';
    const cartServicePath = 'http://localhost:8889/webapp-cart';
        ...
        $http.post(contextPath + '/auth', $scope.user)
            .then(function successCallback (response) {
                if (response.data.token) {
                    ...
                    $http.get(cartServicePath + '/api/v1/cart/' + $localStorage.webappGuestCartId + '/merge')
                        .then(function successCallback (response) {
                            $localStorage.webappGuestCartId = response.data.value
                        })
                }
